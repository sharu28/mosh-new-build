...
import { Clock, CheckCircle2, Heart, ShieldAlert, CornerDownRight } from 'lucide-react';

interface SlimmingPageProps {
  setCurrentPage: (page: Page) => void;
  setPreselectedServiceId: (id: string) => void;
}

export default function SlimmingPage({ setCurrentPage, setPreselectedServiceId }: SlimmingPageProps) {
  // Only display SLIMMING category services from database
  const slimmingServices = SERVICES.filter(s => s.category === 'slimming');
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-12" id="slimming-sitemap-page">
      {/* 1. Header Section */}
      <div className="text-left mb-16 max-w-3xl">
        ...
        <h1 className="font-serif text-4xl md:text-5.5xl font-normal text-charcoal-deep leading-tight mt-4">
          ...
          <span className="italic font-serif text-salon-accent">sculpt a sharp modern silhouette</span>
        </h1>
        <p className="text-xs text-charcoal-light font-sans mt-4 max-w-xl leading-relaxed">
          Welcome to Gosh-approved medical-grade non-surgical fat crystallization, ultrasonic fat emulsification, and neuromuscular stimulation. Melt stubborn centimeters and restore cell density safely.
        </p>
      </div>
      ...
      {/* 3. Slimming Services Cards Grid */}
      <div className="space-y-16">
        {slimmingServices.map((service, index) => (
          <div 
            key={service.id}
          ...
          >
            ...
            <div className="col-span-12 lg:col-span-6 text-left space-y-6">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-salon-accent block font-sans">
                TECHNOLOGY BODY SLIMMING PILLAR
              </span>
              ...
              <div className="space-y-3 py-2">
                <div className="flex items-start gap-2.5 text-xs text-charcoal-deep font-sans">
                  ...
                  <div><strong>Clinically Certified Devices:</strong> Targeted inch reduction with zero down-time.</div>
                </div>
                <div className="flex items-start gap-2.5 text-xs text-charcoal-deep font-sans">
                  <CheckCircle2 className="w-4 h-4 text-salon-accent shrink-0 mt-0.5" />
                  <div><strong>Metabolic Optimization:</strong> Safely triggers natural adipose cell drainage systems.</div>
                </div>
              </div>
      ...
      <div className="mt-28 bg-white border border-peach-dark rounded-3xl p-8 md:p-12 text-center relative overflow-hidden">
        <div className="max-w-xl mx-auto flex flex-col items-center gap-4">
          ...
          <h2 className="font-serif text-2xl md:text-3xl font-normal text-charcoal-deep leading-tight">Start your slim transformation session</h2>
          <p className="text-xs text-charcoal-light font-sans leading-relaxed">
            Unsure of which advanced fat reduction apparatus matches your skin health goals or previous histories? Register for our risk-free introductory contouring counseling.
          </p>
          <button
            onClick={() => handleBookNow('slimming-cavitation')}
          ...
          >
            Schedule Cavitation Test (Rs. 18,000)
          </button>
        </div>
      </div>
    </div>
